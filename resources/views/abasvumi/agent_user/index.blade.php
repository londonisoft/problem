...
<div class="content-wrapper">
                  ...
                  <table class="table table-bordered table-striped table-sm">
                    ...
                    <tbody id="allUser">
                      @foreach($agents as $agent)
                        <tr align="center" class="user-{{ $agent->id }}">
                            <td>{{ $agent->name }}</td>
                            <td>{{ $agent->email }}</td>
                            <td>{{ $agent->phone }}</td>
        ...
        <div class="modal-dialog modal-md" User="document">
            <div class="modal-content">
                <div class="modal-header bg-success text-white">
                  <h5 class="modal-title text-center w-100">Create New Agent</h5>
    ...
    <div class="modal fade" tabindex="-1" id="editUserModal" User="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-md" User="document">
            <div class="modal-content">
                <div class="modal-header bg-success text-white">
                  <h5 class="modal-title text-center w-100">Edit Agent</h5>
                  <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                  </button>
                </div>
...
@endsection
@section('scripts')
<script src="{{ asset('abasvumi/admin/js/agent.js') }}"></script>
<script>    
    $(".users").addClass("menu-open");
    $(".nav-agent").addClass("active");
</script>
@endsection